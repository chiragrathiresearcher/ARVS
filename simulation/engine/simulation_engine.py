...
class SimulationEngine:
    ...
    def run(self,
        ...
        # ── Summary statistics ────────────────────────────────────────────
        # Guard: if source returned no frames (e.g. SPICE/JPL unavailable in CI)
        # skip gracefully instead of crashing on empty arrays.
        if not steps:
            logger.warning("No frames produced — skipping (source unavailable)")
            return SimulationResult(
                scenario       = config.name,
                source         = "UNAVAILABLE",
                total_frames   = 0,
                steps          = [],
                summary        = {"pass": True, "skipped": True,
        ...
        if config.scenario_type.value == "normal_ops":
            # Allow up to 5 gate blocks in normal ops — some telemetry sources
            # (e.g. ESA MEX) have elevated thermal profiles that legitimately
            # trigger the safety gate without indicating a fault.
            if n_blocks > 5:
                scenario_pass  = False

...
def run(source_filter=None, scenario_filter=None):
    ...
    for source_name, loader in sources.items():
        ...
        # Skip source entirely if no frames returned (e.g. network unavailable in CI)
        if not frames:
            logger.warning(f"   Source {source_name} returned 0 frames — skipping all scenarios")
            continue
    ...
    overall_pass  = all(r.pass_fail for r in all_results
                        if not r.summary.get("skipped", False))
